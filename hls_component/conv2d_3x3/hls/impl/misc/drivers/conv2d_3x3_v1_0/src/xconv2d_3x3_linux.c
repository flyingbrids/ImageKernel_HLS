...
typedef struct {
    ...
    XConv2d_3x3_uio_map maps[ MAX_UIO_MAPS ];
} XConv2d_3x3_uio_info;

/***************** Variable Definitions **************************************/
static XConv2d_3x3_uio_info uio_info;
...
static int uio_info_read_name(XConv2d_3x3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XConv2d_3x3_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XConv2d_3x3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XConv2d_3x3_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XConv2d_3x3_Initialize(XConv2d_3x3 *InstancePtr, const char* InstanceName) {
	XConv2d_3x3_uio_info *InfoPtr = &uio_info;
