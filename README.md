...
In the output-packetized branch, it creates m_axi memory map interface as both input and output instead stream interface. The m_axi can be connected to external DDR3/4 memory as well as interal BRAM. This will allow the IP to be directly deployed to the SoC platform and it will read in the image and write out the filtered image to the memory location determined by the axi-lite bus. The flow control is also dictated by register bits accessible from axi-lite bus.
